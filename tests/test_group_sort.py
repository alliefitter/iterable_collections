...
class TestGroupSort(unittest.TestCase):
    ...
    def test_lists(self):
        ...
        c = collect(test).sorted_and_groupby(key=lambda x: x[3]) \
    ...
    def test_dicts(self):
        ...
        c = collect(test).sorted_and_groupby(key=lambda x: x['c']) \
            .map(lambda x: (x[0], list(x[1])))
        self.assertEqual(
            c.list(),
    ...
    def test_tuples(self):
        ...
        c = collect(test).sorted_and_groupby(key=lambda x: x[3][1]) \
            .map(lambda x: (x[0], list(x[1])))
        self.assertEqual(
            c.list(),
