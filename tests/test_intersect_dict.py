...
class TestIntersectDict(unittest.TestCase):

    def test_list_and_list(self):
        c = collect(list(range(10)))
        with self.assertRaises(ValueError):
            c.intersect_dict(list(range(0, 5)))

    def test_list_and_set(self):
        c = collect(list(range(10)))
        with self.assertRaises(ValueError):
            c.intersect_dict(set(range(0, 5)))

    def test_list_and_tuple(self):
        c = collect(list(range(10)))
        with self.assertRaises(ValueError):
            c.intersect_dict(tuple(range(0, 5)))

    def test_list_and_iter(self):
        c = collect(list(range(10)))
        with self.assertRaises(ValueError):
            c.intersect_dict(iter(range(0, 5)))

    def test_list_and_dict(self):
        c = collect(list(range(10)))
        with self.assertRaises(ValueError):
            c.intersect_dict({'a': 1, 'b': 2, 'c': 3})

    def test_list_and_dict_keys(self):
        c = collect(list(range(10)))
        with self.assertRaises(ValueError):
            c.intersect_dict({'a': 1, 'b': 2, 'c': 3}.keys())

    def test_list_and_dict_items(self):
        c = collect(list(range(10)))
        with self.assertRaises(ValueError):
            c.intersect_dict({'a': 1, 'b': 2, 'c': 3}.items())
    ...
    def test_dict_and_dict(self):
        c = collect({'a': 1, 'b': 2, 'c': 3}).intersect_dict({'a': 2, 'd': 4, 'e': 5})
        self.assertEqual(
            c.iterable,
    ...
    def test_dict_and_dict_items(self):
        c = collect({'a': 1, 'b': 2, 'c': 3}).intersect_dict({'a': 2, 'd': 4, 'e': 5}.items())
        self.assertEqual(
            c.iterable,
